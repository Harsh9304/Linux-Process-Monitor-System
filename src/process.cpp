...
//  Return the process's current state (e.g., R = Running, S = Sleeping, T = Stopped/Suspended)
std::string Process::State() {
  std::ifstream stream(ParserConsts::kProcDirectory + std::to_string(pid_) + ParserConsts::kStatusFilename);
  ...
  if (stream.is_open()) {
    while (std::getline(stream, line)) {
      ...
      if (key == "State:") {
        return value;  // Example values: R, S, T
      }
    }
  }
  return "";
